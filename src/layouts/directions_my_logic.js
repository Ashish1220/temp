...
const ParkingInfo = () => {
  ...
  const get_last_row=(algo)=>{
  
    fetch('http://localhost/api/get_last_row.php') // Replace with your XAMPP API URL
    .then(response => response.json())
    .then(data => {
        setalgo(localStorage.getItem("activated_algo"))
        console.log("Erfa",localStorage.getItem("activated_algo"))
 ...
 const [algo,setalgo]=useState(0)
  useEffect(() => {
    ...
    const intervalId = setInterval(get_last_row(algo), 2000);
  ...
  return (
    <div style={containerStyle}>
      ...
      {parkingStatus === '' && (
        <>
          <div style={infoStyle}>
            No of available spaces: {available_slot} {"lala"}
          </div>
          <div style={infoStyle}>
